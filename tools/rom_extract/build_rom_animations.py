...
OUT_TARGETS = REPO / "tools/rom_extract/pykuma_targets.lua"
...
def targets_lua(doc: dict, move_names: dict) -> str:
    """The rip session's shopping list, as a Lua table `dump_cels.lua` reads.

    Two kinds of target, both keyed by what the Lua can see live:
      * `cels`  -- cel ids an animation needs but the manifest does not have
                   (the hit-branch cels a whiff pass never shows, the reaction
                   cels), so the moment one appears it is ripped and ticked off;
      * `anims` -- ROM scripts with no animation at all: a mapped move that was
                   never performed in any capture.
    Generated by `--targets-lua`; the Lua just displays and counts it.
    """
    cels, anims = {}, {}
    for anim, e in doc["anims"].items():
        label = e.get("role") or (e.get("state", "") + (f":{e['variant']}" if e.get("variant") else ""))
        for cel in e.get("missing_cels", []):
            cels[int(cel)] = f"{label} ({anim})"
    for rom_id, info in sorted(move_names.items()):
        if rom_id in doc["anims"]:
            continue
        label = info["state"] + (f":{info['variant']}" if info.get("variant") else "")
        anims[int(rom_id, 16)] = f"{label} ({rom_id}) -- never performed"
    lines = ["-- GENERATED by tools/rom_extract/build_rom_animations.py --targets-lua.",
             ...
             "return {", "  cels = {"]
    for cel, label in sorted(cels.items()):
        lines.append(f'    [{cel}] = "{label}",')
    lines += ["  },", "  anims = {"]
    for anim, label in sorted(anims.items()):
        lines.append(f'    [{anim}] = "{label}",')
    lines += ["  },", "}", ""]
    return "\n".join(lines)
...
def main(argv=None):
    ...
    ap.add_argument("--targets-lua", nargs="?", const=str(OUT_TARGETS), default=None,
                    metavar="PATH",
                    help="also write the rip session's remaining targets (missing cels + moves "
                         f"never performed) as a Lua table for dump_cels.lua (default {OUT_TARGETS.name})")
    ...
    if args.targets_lua:
        ...
        Path(args.targets_lua).write_text(targets_lua(doc, move_names))
        n_cels = sum(len(e.get("missing_cels", [])) for e in doc["anims"].values())
        n_moves = len([r for r in move_names if r not in doc["anims"]])
        print(f"{n_cels} missing cels + {n_moves} never-performed moves -> {args.targets_lua} "
              f"(copy it next to dump_cels.lua)")
